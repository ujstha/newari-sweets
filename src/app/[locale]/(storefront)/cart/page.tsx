...
export default function CartPage() {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 pt-10 pb-28 sm:px-8 sm:py-14">
      <h1 className="font-display text-3xl font-semibold text-ink">Cart</h1>

      <div className="mt-6 flex flex-col gap-8 sm:grid sm:grid-cols-[1.5fr_1fr] sm:items-start">
        <div className="card-surface divide-y divide-border-warm">
          {items.map((item) => (
        ...
        {/* Live order-summary panel -- sticky at sm+, mirrors checkout's
            panel so the running total stays visible without scrolling back
            up. Mobile keeps the sticky bottom bar below instead. */}
        <div className="order-summary-panel sticky top-24 hidden sm:block">
          ...
          <div className="mt-2 flex justify-between text-sm text-ink-soft">
            <span>Payment</span>
            <span>Cash / MobilePay on pickup</span>
          </div>
          ...
          <p className="mt-3 text-xs text-ink-faint">
            Submitting an order sends a request — it isn&apos;t confirmed until we approve it.
          </p>
        </div>
      </div>

      {/* Sticky mobile summary bar -- same pattern as the product page's buy
          bar. Deliberately not applied to /checkout itself: that page's real
          submit button already has to run through required-field
          validation, so a floating shortcut there would compete with it
          rather than help. */}
      <div
      ...
      >
        ...
        <Link href="/checkout" className="btn-primary flex-1 py-2.5 text-center">
          Checkout
        </Link>
      </div>
    </main>
