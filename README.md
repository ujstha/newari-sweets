...
**Build-order Phases 1–6 are done and merged to `dev`.** Repo/tooling/i18n/domain layer (Phase 1); auth + master data (Phase 2); site content CMS (Phase 3); catalog authoring (Phase 4); public storefront catalog (Phase 5); and cart + checkout + order creation (Phase 6) — `orders`/`order_items`/`order_item_options`/`order_dietary_requests` + RLS, the `create_order()` and `get_order_status()` RPCs, a real checkout form (fulfillment, per-cart minimum-lead-time date floor, dietary multiselect, terms acceptance), and the `/orders/[orderId]` status page. All tested against the real Supabase project, including server-side re-pricing (the server never trusts the client's cart prices) and the delivery-fields-required DB constraint.

Best Sellers is still not built -- `orders` now exists, so `getBestSellingProducts()` can be added to `public-catalog.ts` and wired into the homepage whenever convenient; it just wasn't the focus of this phase.

See PLAN.md's "Phased Build Order" for what phases 7–10 cover next (Phase 7, Admin order review workflow, is up next). This is being built autonomously, phase by phase, without stopping for review between phases — see the note at the bottom of this file for what to check when you're back.
...
Check PLAN.md's "Phased Build Order" for what's next. Phase 7 (Admin order review workflow) is next — the order review queue and decision screen (approve/decline/ready/complete/cancel), `order-workflow.ts`'s `canTransition()` guard wired into the Server Actions doing these updates, a matching DB trigger as defense-in-depth, and the admin order-anonymize action for GDPR erasure requests. Also worth doing then: add `getBestSellingProducts()` to `public-catalog.ts` and wire the Best Sellers homepage section, now that `orders` exists. Each phase is scoped to end in something concretely testable end-to-end, per PLAN.md's Verification section — don't skip ahead to later phases' UI/features before their data model exists.
...
**Recurring Playwright test-script trap (Phases 4 and 6, not app bugs either time)**: a CSS selector that's ambiguous across multiple near-identical elements silently targets the WRONG one instead of erroring -- once renamed the wrong category via `form:last-of-type` (matches per-parent, not page-wide) when 4 near-identical row forms existed, and once left an order's Terms-of-Sale checkbox unchecked (silently blocking native HTML5 form submission) because `input[type="checkbox"]` matched an earlier dietary-restriction checkbox instead. Both looked like real bugs until checked against a screenshot. Lesson: when a page has multiple structurally-similar elements (repeated rows, multiple checkboxes), select by something unique to the target (placeholder text, a wrapping label's text, a test id) -- never a positional/structural selector like `:last-of-type` or a bare tag+type selector -- and take a screenshot before concluding something didn't work.
